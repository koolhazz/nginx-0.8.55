...
#if (NGX_HAVE_FILE_AIO) /* 是否使用内核异步io*/
        {
        int                 n;
...
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    ee.events = events | (uint32_t) flags; /* flags 应该是EPOLLET OR EPOLLLT */
    ee.data.ptr = (void *) ((uintptr_t) c | ev->instance); /* 加入stale event的判断 */
