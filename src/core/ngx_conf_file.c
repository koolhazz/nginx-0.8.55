...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        type = parse_block; /* {} 分析这里 */

    } else {
        type = parse_param; /* 分析配置项 */
    }
    ...
    for ( ;; ) {
        ...
        if (cf->handler) { /* 判断对应的配置项的handler是否为空 */
            ...
            rv = (*cf->handler)(cf, NULL, cf->handler_conf); /* 初始化 */
            if (rv == NGX_CONF_OK) {
                continue;
            }
...
ngx_conf_set_str_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    field = (ngx_str_t *) (p + cmd->offset); /* 配置项 */
