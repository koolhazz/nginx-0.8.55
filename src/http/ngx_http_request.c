...
void
ngx_http_init_connection(ngx_connection_t *c) /* http 连接初始处理 */
{
    ngx_event_t         *rev;
    ...
    rev = c->read;
    rev->handler = ngx_http_init_request; /* 这里设置读事件的处理函数 */
    c->write->handler = ngx_http_empty_handler; /* 写事件处理函数 就是啥不干*/
    ...
    if (rev->ready) {
        ...
        ngx_http_init_request(rev); /* 如果这里有数据到达直接调用函数进行处理 */
        return;
    }

    ngx_add_timer(rev, c->listening->post_accept_timeout); /* 添加定时器处理，在超时间内没有发送请求数据，使用函数梳理 */

    if (ngx_handle_read_event(rev, 0) != NGX_OK) { /* 添加读事件到epoll中 */
#if (NGX_STAT_STUB)
        (void) ngx_atomic_fetch_add(ngx_stat_reading, -1);
#endif
...
ngx_http_init_request(ngx_event_t *rev)
{
    ...
    if (rev->timedout) { /* 是否超时 阈值时间内没有收到请求数据 */
    ...
    c->data = r; /* 将ngx_http_request_t 放入到connection 的data域 */
    r->http_connection = hc; /* 反向指向connection */
    ...
    rev->handler = ngx_http_process_request_line; /* 设置读事件 */
    r->read_event_handler = ngx_http_block_reading; 
	/*上面有个read_event_handler的设置，ngx_http_block_reading函数主要是移除read事件。这样做的目的是，
	是为了在处理当前的request时间内，暂时停止处理新的请求，因为同一连接内会有有多个所谓的pipeline请求。
	而什么时间来继续处理新请求，由程序来控制，需要的时候再处理(如在keepalive相关处理)。*/
    ...
    rev->handler(rev); /* call ngx_http_process_request_line */
}
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    rc = NGX_AGAIN; /* 这里就是状态机的阈值，是否再次进行读取 */

    for ( ;; ) {

        if (rc == NGX_AGAIN) {
            n = ngx_http_read_request_header(r); /* 读取头部 */

            if (n == NGX_AGAIN || n == NGX_ERROR) { /*  头部不完整，退出 */
                return;
            }
        }

        rc = ngx_http_parse_request_line(r, r->header_in); /* 解析请求 */
        ...
        if (rc == NGX_OK) {
            ...
            rev->handler = ngx_http_process_request_headers; /* 设置下个步骤的处理函数 */
            ngx_http_process_request_headers(rev); /* 处理http headers */
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            ...
            n = ngx_http_read_request_header(r);/* 读取http header */
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
            ...
            rc = ngx_http_process_request_header(r); /* 处理http header */
...
ngx_http_request_handler(ngx_event_t *ev)
{
    ...
    if (ev->write) {
        r->write_event_handler(r); /* ngx_http_core_run_phases or ngx_http_request_empty_handler */
    } else {
        r->read_event_handler(r); /* 这里什么都不做 */
    }
