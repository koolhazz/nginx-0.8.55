...
static char *
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) /* 解析 http{}  */
{
    char                        *rv;
    ngx_uint_t                   mi, m, s;
    ngx_conf_t                   pcf;
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    for (m = 0; ngx_modules[m]; m++) {
        ...
        ngx_modules[m]->ctx_index = ngx_http_max_module++; /* 模块同类型索引 */
    }
...
ngx_http_add_listening(ngx_conf_t *cf, ngx_http_conf_addr_t *addr)
{
    ...
    ls->handler = ngx_http_init_connection; /* http 连接的处理函数 */
