...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    ngx_start_worker_processes(cycle, ccf->worker_processes,/* 配置的worker数量 */
                               NGX_PROCESS_RESPAWN); //开始worker的工作
...
ngx_start_worker_processes(ngx_cycle_t *cycle, ngx_int_t n, ngx_int_t type)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        ngx_spawn_process(cycle, ngx_worker_process_cycle, NULL,
                          "worker process", type);//生成worker 金车功能
...
static void
ngx_worker_process_cycle(ngx_cycle_t *cycle, void *data) //worker 主循环
{
    ngx_uint_t         i;
    ngx_connection_t  *c;
    ...
    ccf = (ngx_core_conf_t *) ngx_get_co
		nf(cycle->conf_ctx, ngx_core_module);
    ...
    for ( ;; ) {
        ...
        ngx_process_events_and_timers(cycle);//处理事件与定时器
